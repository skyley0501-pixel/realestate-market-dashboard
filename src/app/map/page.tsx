    ...
    <div className="mx-auto max-w-5xl px-4 py-8">
      ...
      {disasterHistoryResult.match(
        (histories) => {
          ...
          const occurredOnList = [...dtos.map((d) => d.occurredOn)].sort();
          const oldestYear = occurredOnList[0].slice(0, 4);
          const newestYear = occurredOnList[occurredOnList.length - 1].slice(0, 4);

          return (
            <section className="mt-10 border-t pt-8">
              <h2 className="mb-2 text-xl font-bold">ハザードマップ（過去の水害・土砂災害履歴）</h2>
              <p className="mb-1 text-sm text-muted-foreground">
                対象範囲: {oldestYear}年〜{newestYear}年に発生した履歴{dtos.length}件（1都3県）
              </p>
              <p className="mb-4 text-sm text-muted-foreground">
                浸水・がけ崩れ・地すべり等の履歴です。地図上のマーカー・範囲をクリックすると詳細が表示されます。
              </p>
