...
const TRENDS_CACHE = cacheHeaders(3600, 86400);
...
export async function GET(req: NextRequest) {
  ...
  const useCase = marketContainer.getTrendsUseCase();
  const result = await useCase.execute({ codes });

  return result.match(
    (snapshots) => NextResponse.json({ data: snapshots.map(toAreaSnapshotDto) }, { headers: TRENDS_CACHE }),
