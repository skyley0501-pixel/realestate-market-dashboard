...
**ゴール**: Phase1〜6の計画外だが、市況の変化やユーザーからの要望に応じて追加した機能を記録する（継続的に追記されるフェーズのため、Phase 4完了・公開後も随時追加される）。

| Day | タスク | 実装内容 | 完了条件 | コミット例 |
|---|---|---|---|---|
| 71 | 金利動向ページの追加 | 長期金利上昇局面を踏まえ、財務省「国債金利情報」CSVから国債10年利回りを取得するバッチ、日銀政策金利の手動シード、両者を重ねたグラフを表示する`/rates`ページを追加 | 金利推移グラフが表示され、本番でも実データが確認できる | `feat(interest-rate): add JGB 10-year yield and BOJ policy rate tracking` |
...
| 77 | 災害履歴レイヤーの表示不具合修正 | 本番で地図は表示されるが点・面のレイヤーが描画されないとのフィードバックを受け対応。(1) MapLibreの`["geometry-type"]`式に依存していたfilterを`properties.geometryType`＋`["get", ...]`の確実な方式に変更 (2) 実データを調査すると被害範囲ポリゴンが数十m四方など極小で初期ズームでは視認できないと判明し、ズームに関わらず見える中心点マーカー（centroidレイヤー）を追加 (3) ポップアップ文字がページのダークテーマ色を引き継ぎ白飛びしていたため濃色を明示指定。ロジックの単体テストを追加 | 地図上に色分けされたマーカーが表示され、クリックで内容が読める | `fix(hazard): render point/polygon disaster history layers reliably` / `fix(hazard): add always-visible centroid markers for tiny disaster polygons` / `fix(hazard): fix hard-to-read popup text color on disaster history map` |
| 78 | 災害履歴の区域外データ除外 | 「千代田区のページに他区のマーカーが付いている」との指摘を受け調査。取得方式（代表点周辺3x3タイル、約30km四方）が面積の小さい市区町村では隣接エリアのデータまで拾っていたと判明し、市区町村の区域ポリゴン（kanto-municipalities.geojson）を使い実際にその区域内にある履歴だけへ絞り込む処理を追加（洗い替え）。レイキャスティング法による点-多角形判定を`src/shared/domain/geo/geometry-utils.ts`に共通ユーティリティとして新設 | 各エリアの地図・一覧に自エリア内の履歴のみ表示される | `fix(hazard): filter disaster history to actual municipality boundaries` |
| 79 | 国土地理院個別災害データの追加 | 「令和以降の最新データが無い」との指摘を受け調査。既存データ（国土調査「土地履歴調査」）は調査時点（2010年代半ば頃）までの過去文献アーカイブと判明し、国土地理院が大規模災害ごとに個別公開する浸水推定図（ZIP同梱GeoJSON）を取り込む仕組みを新設。第1弾として令和元年東日本台風（2019-10-12）を追加し、川越市等に反映 | 令和以降の災害が地図・一覧に表示され、本番でも実データが確認できる | `feat(hazard): add recent disaster data from GSI individual event releases` |
| 80 | ダッシュボードページの整理 | 117市区町村分のチェックボックスが常に全展開でごちゃついていたエリア選択を都道府県別アコーディオンに変更。その上で、ダッシュボードページの中身がホーム・トレンド分析・マーケットマップとほぼ重複していたため削除し、`/dashboard`はホームへ301リダイレクト。ナビゲーション・各所の導線をホームへ整理 | `/dashboard`アクセスがホームへリダイレクトされ、主要機能への導線は維持される | `feat(dashboard): collapse area selector by prefecture into an accordion` / `refactor: remove dashboard page, redirect to home instead` |
| 81 | マンション市場動向機能の追加、取引データ5年分投入時の不具合対応 | 1都3県の新築分譲マンション供給戸数（e-Stat「建築着工統計調査」）と中古マンション取引価格（自社実取引データを都道府県×四半期で集計）の推移を「関連情報」タブに追加。あわせて取引データを直近5年分（663,860件）に拡張する際、一括投入（`createMany`）がタイムアウトして固まる問題が発生し、5,000件ずつのバッチ投入に変更して解決。さらにマンション市場動向ページがビルド時点のデータで静的固定され、DB更新後も本番に反映されない問題を発見し`force-dynamic`化で修正 | マンション市場動向ページが実データで表示され、取引データが直近5年分本番に反映されている | `feat(condo-market): add new/secondhand condominium supply and price trends` / `fix: マンション市場動向ページのデータ更新が反映されない問題を修正` |
| 82 | ホーム画面の統計表示の改善 | 「最新データが速報値である旨」と「各指標の算出基準」をホーム下部に明記。都心部（例: 港区）と郊外・山間部を同じ平均に混ぜないよう、坪単価・前期比・中央値を都道府県別に算出する方式に変更。取引件数が極端に少ない市区町村（統計的代表性が低いエリア）について、しきい値ごとの影響（除外市区町村数・取引件数への影響度）を確認できるインタラクティブな可視化ツールを作成してユーザーと検討し、5年累計1000件以下の95市区町村を集計・ランキング表示の対象から除外（生のTransactionデータは保持し取引検索では引き続き利用可能） | ホーム・エリアランキング等の表示が都道府県別かつ統計的に代表性のあるエリアのみになり、算出基準が明記されている | `feat: ホーム統計を都道府県別に分け、取引の少ない市区町村を集計対象から除外` |
| 83 | Aboutページに著作権表記を追加 | GitHub上のソースコードを無断でコピーされ類似サイトを作られるリスクに備え、著作権法の根拠条文（第10条・第21条・第27条等）と関連判例（スペース・インベーダー・パートII事件、江差追分事件）を調査した上で、著作権の帰属・GitHub公開が利用許諾を意味しない旨をAboutページ下部に明記 | Aboutページに著作権表記が表示され、本番でも確認できる | `feat: Aboutページに著作権表記を追加` |
| 84 | 「トレンド分析」と「エリア比較」機能の統合・再設計 | ページ名が分かりづらいとの指摘を受け「トレンド分析」を「エリア比較」に改称。需要の低かった旧「エリア比較」（坪単価・中央値・取引件数・前期比の4項目レーダーチャート）を関連コードごと削除し、代わりに本ページで取引件数・平均坪単価・中央値の推移を折れ線グラフ3枚で比較できるよう拡張。あわせてホーム画面の統計セクション（都道府県別動向・算出基準の説明）をこのページへ移動し、モバイルでのホーム画面の情報過多を解消 | 「エリア比較」ページで3指標の推移比較ができ、ホーム画面がスマートフォン幅でもすっきり表示される | `feat: 「トレンド分析」を「エリア比較」にリネームし、旧エリア比較(4項目)を統合` / `refactor: ホーム画面の統計セクションをエリア比較ページへ移動` |
| 85 | マーケットマップへのハザードマップ追加 | エリア詳細ページ経由でしか見られず気づきにくかった災害情報について、1都3県全域を俯瞰できる「ハザードマップ」をマーケットマップページの下部に新設（既存のDisasterHistoryMapの座標変換・配色ロジックを再利用）。あわせて表示期間を精査し、1980年より前の古い履歴240件を削除して直近の履歴（147件）に整理、見出しと地図の間に対象範囲（年数・件数）を明記する表示を追加 | マーケットマップでハザードマップが表示され、対象範囲が明記されている | `feat: マーケットマップにハザードマップ（1都3県全域の水害・土砂災害履歴）を追加` / `feat: ハザードマップの見出しと地図の間に対象範囲を明記` |
...
| Phase 7: その他実装 | 随時 | 計画外の追加機能（金利動向など）を継続的に記録 |
| **合計** | **約70日（1日1コミットペース）+ 随時追加分** | |
